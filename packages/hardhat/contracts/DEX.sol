...
contract DEX {
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        // return eth
        (bool send, bytes memory data) = payable(msg.sender).call{
            value: amountToSwap
        }("");
    ...
    {
        uint256 ethToReturn = amount.mul(address(this).balance) /
            totalLiquidity;
        uint256 tokenToReturn = ethToReturn.mul(
            token.balanceOf(address(this))
        ) / address(this).balance;
